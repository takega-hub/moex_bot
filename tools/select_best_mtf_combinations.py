...
def run_mtf_backtest(
    ...
    print(f"Символ: {symbol}")
    print(f"Период: {days_back} дней")
    print(f"Начальный баланс: {initial_balance:.2f} RUB")
    ...
    # Инициализация клиента и коллектора
    # account_id не обязателен для клиента, если он не передан, клиент сам найдет первый счет
    ...
    if not instrument:
        print(f"❌ Инструмент {symbol} не найден")
        return None
    ...
    # Нормализация колонок (time -> timestamp)
    ...
    # Добавляем колонку figi если нет (нужна для некоторых функций)
    ...
    try:
        # Используем встроенный feature_engineer стратегии
        ...
        print("   Используем feature_engineer.create_technical_indicators...")
    ...
    simulator._base_order_usd = None # Используем % от баланса (settings.risk.margin_pct_balance)
    ...
    min_bars_required = 800  # Увеличиваем окно для корректного расчета 1h индикаторов (200 * 4 = 800)
    ...
    for idx in range(min_window_size, len(df_with_features)):
        try:
            ...
            try:
                # ОПТИМИЗАЦИЯ: Используем skip_feature_creation=features_created
                # Это значительно ускоряет бэктест (с ~0.6 сек на бар до ~0.01 сек), если фичи созданы
                signal = strategy.generate_signal(
                    ...
                    leverage=leverage,
                    skip_feature_creation=features_created,  # ОПТИМИЗАЦИЯ
                )
...
Примеры:
  ...
  python backtest_mtf_strategy.py --symbol BTCUSDT --days 30 --no-use-best
    ...
    parser.add_argument("--balance", type=float, default=5000.0, help="Начальный баланс")
    parser.add_argument("--risk", type=float, default=0.02, help="Риск на сделку (0.02 = 2%%)")
    parser.add_argument("--leverage", type=int, default=1, help="Плечо (1 для MOEX спот)")
